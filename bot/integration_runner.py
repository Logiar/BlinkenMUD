...
def run_regression_scenarios(session: MudSession, timeout_s: float) -> None:
    """Exercise targeted edge cases called out in code quality review P2."""

    # Long input line should not crash or wedge command parsing.
    long_token = "x" * 768
    long_response = session.send_and_capture_prompt(f"say {long_token}", timeout_s, allow_reject=True)
    if len(long_response.strip()) == 0:
        raise IntegrationAssertionError(
            "Assertion failure: long-input scenario produced empty response. "
            f"context=regression_long_input encountered_tail={long_response[-800:]}"
        )

    # Permission boundary: low-level player should be denied immortal-only commands.
    shutdown_response = session.send_and_capture_prompt("shutdown", timeout_s, allow_reject=True)
    assert_any_contains(
        shutdown_response,
        ["huh?", "not", "only", "immortal", "security"],
        context="regression_permission_check",
        details={"command": "shutdown"},
    )

    # Telnet-like control characters embedded in input should not desynchronize parsing.
    telnet_like = session.send_and_capture_prompt("say \xff\xfb\x01 integration", timeout_s, allow_reject=True)
    if len(telnet_like.strip()) == 0:
        raise IntegrationAssertionError(
            "Assertion failure: telnet-like input regression produced empty response. "
            f"context=regression_telnet_like encountered_tail={telnet_like[-800:]}"
        )
...
def run_bot(host: str, port: int, timeout_s: float, name: str, password: str, sweep_duration_s: float) -> None:
    ...
    try:
        ...
        # Targeted regression scenarios (long input, malformed telnet, permissions).
        run_regression_scenarios(session, timeout_s)
